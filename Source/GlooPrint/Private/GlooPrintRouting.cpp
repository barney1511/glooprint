...
constexpr int32 MaxQueryCells = 256;
...
public:
    ...
    void Add(FBox2f Box, int32 Node)
    {
        ...
        if (CountCells(Min, Max) > MaxQueryCells) { Large.Add(Index); return; }
    ...
    template<typename Predicate> bool Query(const FBox2f& Box, Predicate Accept) const
    {
        ...
        if (CountCells(Min, Max) > MaxQueryCells || bSmallerThanSpill)
        {
            if (int64(Last) - First + 1 <= MaxQueryCells)
            {
                ...
                for (int32 Index : Wide) { if (!Accept(Index)) { return false; } }
        ...
        if (++Stamp == 0) { Visited.Init(0, Items.Num()); ++Stamp; }
        for (int32 Index : Large) { if (!Accept(Index)) { return false; } }
        for (int32 X = Min.X; X <= Max.X; ++X)
        {
